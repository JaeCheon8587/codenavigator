...
| 버전 | 0.1 (Draft) |
| 작성 가정 | AI 코딩 에이전트 전용 도구. UI 없음. MVP 5개 + delete(F006) 6개 기능 정의. |
...
|---|---|
| App명 | CodeNavigator |
| 역할 | AI 코딩 에이전트용 C# 클래스 시맨틱 인덱서 Python CLI |
| 목적 | 자연어 키워드로 C# 클래스 후보를 좁혀 에이전트의 Grep 시간·토큰 절감 |
| 주요 기능 범위 | 클래스 검색(F001) · reindex(F002) · hook 갱신(F003) · status(F004) · AI description(F005) · 파일 단위 삭제(F006) |
| 범위 밖 | UI, C# 외 언어, 실시간 파일 감시 데몬, embedding 벡터 검색 |
