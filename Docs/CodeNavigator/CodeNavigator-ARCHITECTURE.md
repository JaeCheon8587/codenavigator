...
| 작성 가정 | Python 3.11+ 단일 패키지. DDD 레이어 모델 미적용 (Python CLI 도구). 솔루션 ARCHITECTURE 는 단일 App 레포이므로 미사용. |
...
|---|---|
| App 코드 | CodeNavigator |
| 한 줄 설명 | AI 코딩 에이전트용 C# 클래스 시맨틱 인덱서 Python CLI |
| TFM/런타임 | Python 3.11+ |
| 진입점 경로 | `src/codenav/__main__.py` (`main()`) |
| 호스트 종류 | Python CLI (`python -m codenav` 또는 `codenav` 스크립트) |
